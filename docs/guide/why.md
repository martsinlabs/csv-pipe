...
- **Encodes and parses.** `stringify` and `parse` are mirror images, both typed
  and streaming, so encode then parse round-trips your rows, with dynamic typing
  to recover numbers and booleans.
- **Runs where your code runs.** The core imports no `fs` and no DOM, and
  ...
  Deno, Bun, and edge.
- **Flat memory.** Parsing streams one record at a time, and encoding does too
  once the columns are declared, so a file or HTTP body of any size stays at flat
  memory.
